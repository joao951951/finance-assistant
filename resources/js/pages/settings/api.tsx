...
export default function ApiSettings({
    ...
    function removeKey(e: React.FormEvent) {
        ...
        if (
            confirm(
                'Remover a chave da API? O chat com IA ficará indisponível até você adicionar uma nova.',
            )
        ) {
            removeForm.delete(ApiController.destroy.url(), {
                preserveScroll: true,
            });
        }
    }
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            <SettingsLayout>
                <div className="space-y-6">
                    ...
                    {/* Status da chave */}
                    <div
                        className={`rounded-lg border px-4 py-3 text-sm ${hasApiKey ? 'border-green-200 bg-green-50 text-green-800 dark:border-green-800 dark:bg-green-950 dark:text-green-300' : 'border-yellow-200 bg-yellow-50 text-yellow-800 dark:border-yellow-800 dark:bg-yellow-950 dark:text-yellow-300'}`}
                    >
                        {hasApiKey ? (
                            <span>
                                ✓ Chave de API configurada — os recursos de IA
                                estão disponíveis
                            </span>
                        ) : (
                            <span>
                                ⚠ Nenhuma chave configurada — adicione sua chave
                                OpenAI para ativar os recursos de IA
                            </span>
                        )}
                    </div>
                    ...
                    <form onSubmit={submit} className="space-y-6">
                        ...
                        <div className="grid gap-2">
                            ...
                            <select
                                ...
                                onChange={(e) =>
                                    form.setData('chat_model', e.target.value)
                                }
                        ...
                        <div className="grid gap-2">
                            ...
                            <p className="text-xs text-muted-foreground">
                                Atenção: trocar o modelo de embeddings invalida
                                os embeddings existentes (precisam ser
                                regerados).
                            </p>
                        </div>
                    ...
                    {hasApiKey && (
                        <div className="rounded-lg border border-destructive/30 p-4">
                            ...
                            <p className="mt-1 text-xs text-muted-foreground">
                                Remove permanentemente sua chave e desativa os
                                recursos de IA.
                            </p>
